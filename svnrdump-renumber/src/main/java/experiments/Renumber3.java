...
public class Renumber3 {
  ...
  private static void processDumpFiles(OutputStream os, Path root) throws IOException {
    ...
    for (Path revDump : revDumps) {
      long currentRevision = revNumFromFileName(revDump);

      while (previousRevision + 1 < currentRevision) {
        previousRevision++;
        LinkedHashMap<String,String> headers = new LinkedHashMap<>();
        LinkedHashMap<String,String> props = new LinkedHashMap<>();
        
        headers.put("Revision-number", Long.toString(previousRevision));
        props.put("svn:date", "2000-01-01T00:00:00.000000Z");

        new Block(headers, props, new byte [0]).writeTo(os);
      }
      ...
      visitDumpFile(revDump, (block) -> {
        try {
          ...
          if (block.headers.containsKey("Revision-number")) {
            if (Long.parseLong(block.headers.get("Revision-number")) != revNumFromFileName(revDump)) {
              throw new RuntimeException("Rev num mismatch: " + revDump);
            }
          }
          
          if (block.headers.containsKey("SVN-fs-dump-format-version") ||
              block.headers.containsKey("UUID")) {
            return;
          }
  ...
  static class Block {
    ...
    public void writeTo(OutputStream os) throws IOException {
      ...
      if (props.length > 0) {
        os.write(ascii("Prop-content-length: " + (props.length)));
        os.write('\n');
      }
      if (props.length + body.length > 0) {
        os.write(ascii("Content-length: " + (props.length + body.length)));
        os.write('\n');
      }
